...
1. **[Cloud Shell]** Set the Project IDs for the new or existing projects, if
   ...
   ```
1. **[Cloud Shell]** Enable additional configuration for GCE with manual load
   balancers
   ```
   export ABMRA_BASE_CONF=hybrid-manual-lb
   export ABMRA_ADDITIONAL_CONF=gce
   ```
1. **[Cloud Shell]** Change directory into `gdc-solutions`
   ```
...
1. **[Admin Host]** Set the Project IDs for the projects, these should match the
   ...
   export ABMRA_APP_PROJECT_ID=project-2-bofa-prod
   ```
1. **[Admin Host]** Enable additional configuration for GCE with manual load
   balancers
   ```
   export ABMRA_BASE_CONF=hybrid-manual-lb
   export ABMRA_ADDITIONAL_CONF=gce
   ```
1. **[Admin Host]** Change directory into `gdc-solutions`
   ```
   cd gdc-solutions/software-only/bare-metal/reference-architecture
   ```
